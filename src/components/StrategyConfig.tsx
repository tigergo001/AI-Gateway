  ...
  return (
    ...
    >
      <header className="glass-card rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="font-display text-2xl font-bold text-primary">Auto-Routing Strategy</h2>
          <p className="text-xs text-on-surface-variant font-medium mt-1">Dynamic traffic scheduling based on performance.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <section className="md:col-span-8 bg-white border border-outline-variant rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-8">
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="font-display text-lg font-bold">Sliding Window & Thresholds</h3>
            </div>
            <div className="space-y-10">
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-on-surface uppercase tracking-wider">Analysis Window Size</label>
                  <span className="bg-secondary-container/20 text-on-secondary-container px-3 py-1 rounded-lg text-[10px] font-black font-mono">50 REQUESTS</span>
                </div>
                ...
                <p className="text-[10px] text-on-surface-variant italic opacity-60">Basis for calculating P95 latency.</p>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-on-surface uppercase tracking-wider">TTP Threshold</label>
                  <span className="bg-error-container/20 text-error px-3 py-1 rounded-lg text-[10px] font-black font-mono uppercase">2000MS</span>
                </div>
                ...
                <p className="text-[10px] text-on-surface-variant italic opacity-60">Trigger for automatic degradation logic.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="md:col-span-4 bg-white border border-outline-variant rounded-2xl p-6 shadow-sm space-y-6">
          <div className="flex items-center gap-3">
             ...
             <h3 className="font-display text-lg font-bold">Retry Mechanism</h3>
          </div>
          <div className="space-y-4 pt-2">
            <div className="p-4 bg-surface-container rounded-xl border border-outline-variant/20 shadow-inner">
              <label className="block text-[8px] font-black text-on-surface-variant uppercase tracking-[0.2em] mb-2">Max Retries</label>
              <select className="w-full bg-transparent border-none p-0 font-bold text-primary text-sm focus:ring-0">
                <option>3 Times (Recommended)</option>
                <option>1 Time (Fast Fail)</option>
                <option>5 Times (High Spec)</option>
              </select>
            </div>
            <div className="flex items-center justify-between py-2.5">
              <span className="text-xs font-bold text-on-surface uppercase">Exponential Backoff</span>
              <div 
            ...
            <div className="flex items-center justify-between py-2.5 border-t border-outline-variant/10">
              <span className="text-xs font-bold text-on-surface-variant opacity-60 uppercase">Stream Support</span>
              <div 
        ...
        <section className="md:col-span-12 glass-card rounded-2xl overflow-hidden">
          <div className="px-6 py-4 bg-surface-container-low border-b border-outline-variant/30 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-display text-lg font-bold">Fallback Model Selector</h3>
            </div>
            <span className="text-[9px] font-black bg-white px-3 py-1 rounded-full border border-outline-variant shadow-sm uppercase tracking-widest text-on-surface-variant">ACTIVE: {fallbackModel}</span>
          </div>
            ...
            {[
              ...
              >
                ...
                <div className="mt-4 flex gap-2">
                  <span className="text-[8px] font-black uppercase tracking-widest bg-white border border-outline-variant/50 px-2 py-0.5 rounded shadow-sm">Lat: {m.lat}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="md:col-span-12 bg-primary-container text-on-primary-container rounded-2xl p-6 shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-display text-xl font-bold uppercase tracking-widest">Execution Logic</h3>
            </div>
            <div className="flex flex-col lg:flex-row gap-10">
              <div className="flex-1 space-y-6">
                {[
                  { tag: 'IF', text: 'Primary model P95 latency > 2.0s over 50 requests OR error rate > 5%.' },
                  { tag: 'THEN', text: `System automatically switches to ${fallbackModel} within 200ms.` },
                  { tag: 'RECOVER', text: 'Probing primary path every 5 min; auto-heal if latency < 1.2s.' }
                ].map((item) => (
      ...
      <footer className="fixed bottom-16 left-0 right-0 p-4 flex justify-end gap-3 pointer-events-none">
        <div className="pointer-events-auto flex gap-3 translate-y-12 mb-4">
           <button 
             onClick={() => window.confirm('Discard changes and reset to defaults?') && alert('Reset done.')}
           ...
           <button 
             onClick={() => alert('Strategy configuration saved successfully!')}
